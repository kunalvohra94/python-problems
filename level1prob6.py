...
if __name__ == '__main__':
    ...
    for _ in range(int(input())):
        list_nested.append([input(), float(input())])
    
    for listval in list_nested:
        listval[1]
        if min_val > float(listval[1]):
            smin_val = min_val
            min_val = float(listval[1])
        if smin_val > float(listval[1]) > min_val:
            smin_val = float(listval[1])
       
       
for name in sorted(list_nested):
    if smin_val == name[1]:
        print (name[0])
